...
void __device__ produce_A_tiles(
    ...
    if constexpr (OP_M == 8) {
        produce_4_half_tiles<B_LANES, QSIZE>(A, A_buffer, A_producers, queue, index, p_state, role_id, dropped_rows, k,
                                                          k_blocks);
    } else {
        static constexpr int OP_K = (32 * 16) / OP_M;
        produce_n_full_tiles<1, QSIZE, OP_K, OP_M, true>(A, A_buffer, A_producers, queue, B_LANES, index, p_state,
...
void __device__ produce_B_tiles(
    ...
    produce_n_full_tiles<B_LANES, QSIZE, OP_K, OP_N, false>(B, B_buffer, B_producers, queue, 1, index, p_state, role_id,
